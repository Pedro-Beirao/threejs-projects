...
export const LoadObjects =
{
    ...
    Load(nodes, materials_)
    {
        ...
        let rootObject = new MyObject(rootNode.id);
    ...
    GetLight(node)
    {
        ...
        switch (node["type"])
        {
            ...
            case 'directionallight':
                ...
                light.shadow.camera.left = node.shadowleft ?? -5;
                light.shadow.camera.right = node.shadowright ?? 5;
                light.shadow.camera.bottom = node.shadowbottom ?? -5;
                light.shadow.camera.top = node.shadowtop ?? 5;
                break;
            default:
                break;
        }
        
        light.enabled = node.enabled ?? true;
        ...
        light.shadow.camera.far = node.shadowfar ?? 500;
        light.shadow.mapSize.width = node.shadowmapsize ?? 512;
        light.shadow.mapSize.height = node.shadowmapsize ?? 512;
        light.name = node.id + "_light";
